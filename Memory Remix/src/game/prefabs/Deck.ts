...
export class Deck implements IShuffleGameScene {
  ...
  initCards() {
    ...
    this.cards.forEach((card, index) => {
      ...
      if (position) {
        card.init(position.x, position.y, position.delay, position.scale);
  ...
  getCardsPosition(): {
    ...
    const marginX =
      this.game.configSetting.marginX * this.game.configSetting.marginFactor;
    const marginY =
      this.game.configSetting.marginY * this.game.configSetting.marginFactor;
    ...
    const offsetY = (this.game.scale.height - gridHeight) / 2 + cardHeight / 2;
